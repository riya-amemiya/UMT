...
use umt_rust::date::{UmtDateTime, umt_format, umt_format_iso, umt_get_timezone_offset_string};

#[test]
fn test_format_date_correctly() {
    let date = UmtDateTime::from_ymd_hms(2023, 6, 10, 15, 30, 45).unwrap();
...
fn test_format_day_of_week() {
    let date = UmtDateTime::from_ymd_hms(2023, 6, 10, 0, 0, 0).unwrap();
...
fn test_format_morning_hours() {
    let date = UmtDateTime::from_ymd_hms(2023, 6, 10, 9, 5, 8).unwrap();
