...
#[test]
fn test_throw_on_invalid_utf8_sequence() {
    // 0x80 is invalid UTF-8 start byte, base64 encoded as "gA=="
    let result = umt_from_base64("gA==");
    assert!(result.is_err());
    assert_eq!(result.unwrap_err(), "Invalid UTF-8 sequence");
}
