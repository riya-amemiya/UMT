...
var (
	slugDiacritics  = regexp.MustCompile("[\u0300-\u036F]")
	slugNonWord     = regexp.MustCompile(`[^\w\s-]`)
	slugSpaces      = regexp.MustCompile(`\s+`)
	slugUnderscores = regexp.MustCompile(`_+`)
	slugMultiDash   = regexp.MustCompile(`-+`)
	slugEdgeDash    = regexp.MustCompile(`^-+|-+$`)
)
...
func Slugify(s string) string {
	...
	result = slugDiacritics.ReplaceAllString(result, "")

	// Lowercase
	result = strings.ToLower(result)

	// Replace non-word characters (not [a-zA-Z0-9_\s-]) with dash
	result = slugNonWord.ReplaceAllString(result, "-")

	// Replace whitespace with dashes
	result = slugSpaces.ReplaceAllString(result, "-")

	// Replace underscores with dashes
	result = slugUnderscores.ReplaceAllString(result, "-")

	// Remove multiple consecutive dashes
	result = slugMultiDash.ReplaceAllString(result, "-")

	// Remove leading and trailing dashes
	result = slugEdgeDash.ReplaceAllString(result, "")
...
// Common Latin accented characters decomposition (NFD).
var nfdDecompositions = map[rune]string{
	'\u00C0': "A\u0300", // A with grave
	'\u00C1': "A\u0301", // A with acute
