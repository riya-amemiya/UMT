...
func TestIsNumber_IntegersAndDecimals(t *testing.T) {
	...
	for _, tc := range cases {
		if got := IsNumber(tc.input); got != tc.want {
...
func TestIsString(t *testing.T) {
	...
	for _, tc := range cases {
		if got := IsString(tc.input); got != tc.want {
...
func TestIsArray(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := IsArray(tc.input); got != tc.want {
...
func TestIsDouble(t *testing.T) {
	...
	for _, tc := range cases {
		if got := IsDouble(tc.input); got != tc.want {
...
func TestIsDictionaryObject(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := IsDictionaryObject(tc.input); got != tc.want {
...
func TestParseEmail(t *testing.T) {
	...
	t.Run("extracts parts correctly", func(t *testing.T) {
		parts, err := ParseEmail("user@example.com")
		if err != nil {
		...
		parts2, err := ParseEmail("test.user@sub.example.com")
		if err != nil {
			t.Fatalf("ParseEmail returned error: %v", err)
		}
...
func TestParseEmailWithLevel(t *testing.T) {
	t.Run("basic level", func(t *testing.T) {
		result := ParseEmailWithLevel("user@example.com", "basic")
		if !result.Valid {
	...
	t.Run("validates with different RFC levels", func(t *testing.T) {
		...
		for _, level := range levels {
			result := ParseEmailWithLevel("test@example.com", level)
			if !result.Valid {
	...
	t.Run("basic level rejects invalid", func(t *testing.T) {
		...
		for _, email := range invalidEmails {
			result := ParseEmailWithLevel(email, "basic")
			if result.Valid {
	...
	t.Run("rfc822 allows domains without TLD", func(t *testing.T) {
		result := ParseEmailWithLevel("user@localhost", "rfc822")
		if !result.Valid {
			t.Error("user@localhost should be valid at rfc822 level")
		}
		result = ParseEmailWithLevel("user@localhost", "rfc5321")
		if result.Valid {
	...
	t.Run("rfc2822 requires TLD", func(t *testing.T) {
		result := ParseEmailWithLevel("user@localhost", "rfc2822")
		if result.Valid {
			t.Error("user@localhost should be invalid at rfc2822 level")
		}
		result = ParseEmailWithLevel("user@example.com", "rfc2822")
		if !result.Valid {
	...
	t.Run("rfc2822 rejects consecutive dots", func(t *testing.T) {
		result := ParseEmailWithLevel("user..name@example.com", "rfc2822")
		if result.Valid {
	...
	t.Run("rfc5321 rejects domain with special chars", func(t *testing.T) {
		result := ParseEmailWithLevel("user@exam!ple.com", "rfc5321")
		if result.Valid {
	...
	t.Run("rfc5321 domain literals", func(t *testing.T) {
		result := ParseEmailWithLevel("user@[192.168.0.1]", "rfc5321")
		if !result.Valid {
			t.Error("user@[192.168.0.1] should be valid at rfc5321")
		}
		result = ParseEmailWithLevel("user@[IPv6:2001:db8::1]", "rfc5321")
		if !result.Valid {
	...
	t.Run("rfc5322 allows comments", func(t *testing.T) {
		...
		for _, email := range commentEmails {
			result := ParseEmailWithLevel(email, "rfc5322")
			if !result.Valid {
				t.Errorf("%q should be valid at rfc5322 level", email)
			}
			resultBasic := ParseEmailWithLevel(email, "basic")
	...
	t.Run("rejects completely invalid strings across all levels", func(t *testing.T) {
		...
		for _, str := range invalidStrings {
			for _, level := range levels {
				result := ParseEmailWithLevel(str, level)
				if result.Valid {
	...
	t.Run("rejects malformed domains across all levels", func(t *testing.T) {
		...
		for _, email := range malformedDomains {
			for _, level := range levels {
				result := ParseEmailWithLevel(email, level)
				if result.Valid {
	...
	t.Run("rfc5322 domain literals", func(t *testing.T) {
		result := ParseEmailWithLevel("user@[192.168.1.1]", "rfc5322")
		if !result.Valid {
			t.Error("IP literal should be valid at rfc5322")
		}
		result = ParseEmailWithLevel("user@[IPv6:2001:db8::1]", "rfc5322")
		if !result.Valid {
	...
	t.Run("basic rejects IP literals", func(t *testing.T) {
		result := ParseEmailWithLevel("user@[192.168.0.1]", "basic")
		if result.Valid {
	...
	t.Run("rfc5322 empty quoted string", func(t *testing.T) {
		result := ParseEmailWithLevel(`""@example.com`, "rfc5322")
		if !result.Valid {
	...
	t.Run("punycode addresses", func(t *testing.T) {
		result := ParseEmailWithLevel("user@xn--bcher-kva.com", "basic")
		if !result.Valid {
			t.Error("punycode address should be valid at basic level")
		}
		result = ParseEmailWithLevel("user@xn--bcher-kva.com", "rfc5321")
		if !result.Valid {
