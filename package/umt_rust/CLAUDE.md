...
- Cache fixed `regex::Regex` values in `std::sync::LazyLock` statics. Do not use `OnceLock` for these; the crate already uses `LazyLock` (`umt_strip_ansi`, `umt_strip_tags`, `umt_words`, `umt_normalize_whitespace`, `umt_unescape_html`, `umt_is_absolute_url`, `umt_hexa_to_rgba`, `umt_format` escaped-text, `parse_email`, UA extractors). Caller-built patterns (`format_string`, calculator, `umt_regex_match`) stay inline.
