use umt_rust::date::{
    umt_date_range, umt_format, umt_birthday, umt_day_of_week, umt_get_day,
    umt_is_leap_year, umt_new_date_int, DayLanguage
};
...
fn test_date_range_and_format() {
    ...
    let formatted_dates: Vec<String> = dates.iter()
        .map(|date| umt_format(date, "YYYY-MM-DD", 0))
        .collect();

    assert_eq!(formatted_dates, vec![
        "2025-01-01",
        "2025-01-02",
        "2025-01-03",
        "2025-01-04",
...
fn test_age_and_birthday_format() {
    ...
    let birth_date = umt_new_date_int(birth_year, birth_month, birth_day, None, None, None, None).unwrap();
...
fn test_leap_year_dates() {
    ...
    for year in leap_years {
        ...
        assert_eq!(umt_format(&dates[dates.len() - 1], "YYYY-MM-DD", 0), format!("{}-02-29", year));
    }
