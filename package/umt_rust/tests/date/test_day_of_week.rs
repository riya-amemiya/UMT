...
fn test_day_of_week_for_specific_date() {
    ...
    assert_eq!(umt_day_of_week(Some(2020), Some(1), Some(1), Some(0)), Some(3));
    // January 2, 2020 was a Thursday (day 4)
    assert_eq!(umt_day_of_week(Some(2020), Some(1), Some(2), Some(0)), Some(4));
}

#[test]
fn test_day_of_week_known_dates() {
    // January 1, 2025 is a Wednesday (day 3)
    assert_eq!(umt_day_of_week(Some(2025), Some(1), Some(1), Some(0)), Some(3));

    // January 5, 2025 is a Sunday (day 0)
    assert_eq!(umt_day_of_week(Some(2025), Some(1), Some(5), Some(0)), Some(0));

    // January 4, 2025 is a Saturday (day 6)
    assert_eq!(umt_day_of_week(Some(2025), Some(1), Some(4), Some(0)), Some(6));
}
