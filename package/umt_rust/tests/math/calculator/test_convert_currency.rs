...
fn test_convert_yen_to_base() {
    ...
    rates.insert("¥".to_string(), Value::Float(0.01));
...
fn test_convert_dollar_to_base() {
    let mut rates = HashMap::new();
    rates.insert("$".to_string(), Value::Float(1.2));
...
fn test_convert_euro_to_base() {
    let mut rates = HashMap::new();
    rates.insert("€".to_string(), Value::Float(1.1));
...
fn test_unknown_currency() {
    let mut rates = HashMap::new();
    rates.insert("$".to_string(), Value::Float(1.0));
...
fn test_conversion_with_fractional_rate() {
    let mut rates = HashMap::new();
    rates.insert("£".to_string(), Value::Float(0.5));
