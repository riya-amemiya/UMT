//! Tests for odd number validation

use umt_rust::validate::number::umt_odd;

#[test]
fn test_odd_returns_true_for_odd_numbers() {
...
#[test]
fn test_odd_returns_false_for_even_numbers() {
    let validator = umt_odd(None);
...
#[test]
fn test_odd_returns_custom_message() {
    let custom_message = "This is a custom message";
    let validator = umt_odd(Some(custom_message.to_string()));
    assert!(!(validator.validate)(&2.0));
    assert_eq!(validator.message, Some(custom_message.to_string()));
}

#[test]
fn test_odd_handles_zero() {
    let validator = umt_odd(None);
    assert!(!(validator.validate)(&0.0));
}

#[test]
fn test_odd_handles_negative_numbers() {
    let validator = umt_odd(None);
    assert!((validator.validate)(&-1.0));
    assert!((validator.validate)(&-3.0));
    assert!((validator.validate)(&-5.0));
...
#[test]
fn test_odd_handles_decimal_numbers() {
    let validator = umt_odd(None);
...
#[test]
fn test_odd_handles_large_numbers() {
    let validator = umt_odd(None);
