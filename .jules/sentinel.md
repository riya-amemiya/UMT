...
## 2026-04-05 - Silent Data Corruption in Base58/Base32 Decoding
**Vulnerability:** `decodeBase58` and `decodeBase32` used nullish coalescing (`?? 0`) when looking up character indices, causing invalid characters to silently map to index 0. This produced corrupted output instead of raising an error, which could lead to incorrect address resolution or data integrity failures in cryptographic contexts.
**Learning:** Encoding/decoding functions that operate on restricted alphabets must reject out-of-alphabet characters explicitly. Silent fallback values in lookup operations can mask input corruption and produce plausible-but-wrong results.
**Prevention:** Always check for `undefined` results from Map lookups on restricted alphabets and throw a descriptive error rather than substituting a default value.
